...
func (fr *WriteFramer) ServoConfig(pin byte, max int32, min int32) error {
	return fr.write([]byte{
		START_SYSEX,
		SERVO_CONFIG,
		pin,
...
func (fr *WriteFramer) AnalogWrite(pin byte, value uint32) error {
	return fr.write([]byte{ANALOG_MESSAGE | pin, byte(value & 0x7F), byte((value >> 7) & 0x7F)})
}

func (fr *WriteFramer) ExtendedAnalogWrite(pin byte, value uint32) error {
	b0 := byte(value & 0x7F)
...
func (fr *WriteFramer) I2cWrite(address int32, data []byte) error {
	rs := len(data)*2 + 5
	fr.bufI2C[2], fr.bufI2C[rs-1] = byte(address&0x7F), END_SYSEX

	var byte3 int32 = 0b00000000
...
func (fr *WriteFramer) I2cRead(address int32, autoRestartTransmission bool, continuous bool, numBytes byte) error {
	var byte3 int32 = 0b00001000
	if continuous {
		byte3 = 0b00010000
	}
	addr3 := 0b00000111 & (address >> 7)
...
func (fr *WriteFramer) I2cConfig(delay uint32) error {
	return fr.write([]byte{
		START_SYSEX,
		I2C_CONFIG,
		byte(delay & 0x7F),
...
func (fr *WriteFramer) SamplingInterval(ms uint32) error {
	return fr.write([]byte{
		START_SYSEX,
		SAMPLING_INTERVAL,
		byte(ms & 0x7F),
...
func (fr *ReadFramer) ReadFrame() (f *ReadFrame, err error) {
	...
	case REPORT_VERSION == messageType:
		f = &ReadFrame{
			Type: REPORT_VERSION,
			Data: NewProtocalVersion(FirmataProtocolName, fr.buf[1], fr.buf[2]),
		}
	case ANALOG_MESSAGE <= messageType && messageType <= 0xEF:
		f = &ReadFrame{
			...
			Data: &AnalogPinValueFrameData{
				...
				Value: uint32(fr.buf[1]) | uint32(fr.buf[2])<<7,
			},
		}
	...
	case START_SYSEX == messageType:
		...
		case CAPABILITY_RESPONSE:
			...
			for i, val := range fr.buf[2 : fr.cur-1] {
				if val == 0x7F {
					pins[dx] = &Pin{
						Dx:     dx,
						Name:   pb.PinName_PX, // unkown name
						Modes:  modes,
		...
		case PIN_STATE_RESPONSE:
			state := uint32(fr.buf[4])
			if fr.cur > 6 {
				state = state | uint32(fr.buf[5])<<7
			}
			if fr.cur > 7 {
				state = state | uint32(fr.buf[6])<<14
			}
			if fr.cur > 8 {
				state = state | uint32(fr.buf[7])<<21
			}
			f = &ReadFrame{
		...
		case REPORT_FIRMWARE:
			name := From14bits(fr.buf[4 : fr.cur-1])
			f = &ReadFrame{
				Type: REPORT_FIRMWARE,
				Data: NewFirmwareVersion(string(name), fr.buf[2], fr.buf[3]),
			}
		case STRING_DATA:
			f = &ReadFrame{
				Type: STRING_DATA,
